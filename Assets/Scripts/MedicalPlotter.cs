...
public class MedicalPlotter : MonoBehaviour
{
     ...
     * \brief This file extrapolates the data from the CSVReader file and uses it to attach it to the "Datapoint" prefab. It also plots the datapoint into the game.
    */
    ...
    void Start()
    {
        ...
        synonymColumn = columnNames[synonymId];
        ...
        for (var i = 0; i < dataList.Count; i++)
        {
            ...
            if (dataList[i].ContainsKey(parentColumn))
            {
                ...
                dataPoint.GetComponent<DataContainer>().synonyms = dataList[i][synonymColumn].ToString();
            }
            ...
            foreach (KeyValuePair<string, List<GameObject>> parentChildrenPair in parentMap)
            {
                ...
                if (parentChildrenPair.Key != "null")
                {
                    foreach (GameObject childNode in parentChildrenPair.Value)
                    {
                        ...
                        childNode.transform.position = new Vector3(parent.transform.position.x + counter, numLayer * 20, 0);
    ...
    IEnumerator CirculateNodes(List<GameObject> objects)
    {
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            ...
            Vector3 pos = RandomCircle(new Vector3(0, 0, 0), 200f, angle);
